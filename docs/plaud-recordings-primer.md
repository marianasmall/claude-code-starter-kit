...
- **A wearable records everything**: medical appointments, legal calls, salary conversations, family. If any of the archive is shared (a company Drive), route by recording id with an allow-list for the shared destination, and add a second gate on top: a keyword you put in the title of anything personal (an all-caps PRIVATE or EMBARGO works) that the exporter treats as never-shared. Ids alone miss the recording you forgot to list; a title keyword alone leaks the one you retitled; both together fail closed. Match the keyword case-sensitively, so a "private speakeasy" business recording does not trip it. Decide the ambiguous cases by hand. A sensitive file on a shared drive is not retractable.
